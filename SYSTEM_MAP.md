...
- POST pemeriksaan realisasi memakai `lib/request-origin.ts`: Origin harus cocok dengan alamat publik terkonfigurasi (BETTER_AUTH_URL/NEXT_PUBLIC_APP_URL), bukan alamat internal proxy. Konfigurasi produksi kosong dan origin asing ditolak; `request-origin.test.ts` menguji batas ini.
...
| **Setelan aturan** | `lib/insentif-settings.ts` + tabel `app_setting` — ambang Target AO GT/TT (`fixed240` \| `file`) + pengecualian **per baris per periode** (key `insentif_ao_file:YYYY-MM`, daftar `KODE\|PRINCIPAL`; tombol "Pakai target file" di tabel Input Support Principle → `PATCH /api/insentif-sales/targets/ao-file`, izin `upload_target` + cakupan tim), daftar cabang NILAI_JUAL, daftar SM berhak, dan **semua konstanta uang** (`lib/insentif-konstanta.ts`, key `insentif_konstanta`: pool & bobot GT/MT, ambang bayar, ambang IA MT, rate SPV, strata SM, tarif PPh). Diubah lewat `PATCH /api/insentif-sales/settings` (izin `manage`) dari panel Admin → "Konstanta skema insentif". Gagal baca jatuh ke bawaan `DEFAULT_KONSTANTA`, bukan melempar; fungsi kalkulasi menerimanya sebagai parameter `k`, tanpa `k` memakai bawaan. |
