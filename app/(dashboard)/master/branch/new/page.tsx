...
export default function NewBranch() {
    ...
    const onSubmit = async (data: BranchFormValues) => {
        ...
        try {
            ...
            const response = await accurateFetch("/api/branch/save.do", "POST", payload);
        ...
        <div className="flex flex-col h-full gap-6 pb-12">
            ...
            <div className="flex items-center justify-between bg-[#16181d]/80 rounded-2xl border border-white/5 shadow-2xl p-4 backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-3">
                    <Link href="/master/branch" className="p-2 hover:bg-white/5 rounded-lg transition-colors text-slate-400 hover:text-white">
                        <ArrowLeft size={20} />
                    </Link>
                    <div className="w-px h-6 bg-white/10 mx-2"></div>
                    <div className="flex items-center gap-2">
                        <GitBranch size={18} className="text-indigo-400" />
                        <h1 className="text-lg font-bold text-white tracking-tight">Cabang Baru</h1>
                    </div>
                </div>
