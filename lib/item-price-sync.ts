/*
 * Tujuan: Tarik `item.detailSellingPrice[]` + satuan dari Accurate ke `item_selling_price`/`item`.
 * Caller: scripts/sync-item-selling-price.ts (manual) dan app/api/cron/sync-item-prices (produksi).
 * Dependensi: lib/accurate-session (kredensial), lib/db, db/schema.
 * Main Functions: syncItemPrices — iterasi item, ambil detail, upsert harga + satuan, checkpoint.
 * Side Effects: Tulis `item_selling_price`, `item` (kolom satuan), `sync_state`. Accurate read-only.
 *
 * `item/list.do` tidak membawa priceCategory maupun nama satuan (dibuktikan live 2026-09-08 dan
 * 2026-09-09), jadi keduanya hanya bisa diambil lewat `item/detail.do` PER ITEM. Karena
 * panggilannya sudah terjadi di sini, satuan ikut terisi tanpa panggilan API tambahan.
 *
 * Aman diulang: checkpoint di `sync_state` (module `item_selling_price`) berisi item_id terakhir
 * yang selesai, jadi koneksi putus di tengah tidak memaksa mulai dari nol. `limit` membuat
 * pemanggil bisa memecah pekerjaan panjang menjadi beberapa panggilan pendek — dipakai jalur cron
...
export type SyncItemPricesResult = {
    ok: boolean;
    error?: string;
    processed: number;
    priceRows: number;
...
export async function syncItemPrices(options: SyncItemPricesOptions = {}): Promise<SyncItemPricesResult> {
    const limit = options.limit ?? Infinity;
    const restart = options.restart === true;
    const log = options.log ?? (() => {});
    const started = Date.now();
    const empty = { processed: 0, priceRows: 0, skipped: 0, remaining: 0, cursor: 0, done: false };

    const resolved = await resolveSyncCredentials();
    if (!resolved.creds) {
        return { ok: false, error: resolved.error || "Kredensial Accurate tidak tersedia", ...empty, durationMs: Date.now() - started };
    }
    ...
    async function fetchOne(row: { id: number; no: string }, attempt = 1): Promise<{ prices: PriceRow[]; units: UnitRow } | null> {
        try {
            ...
            log(`  ulang item ${row.id} (percobaan ${attempt + 1}/${RETRIES}) setelah ${wait}ms: ${(error as Error).message}`);
    ...
    for (let start = 0; start < target.length; start += CONCURRENCY) {
        ...
        if (done % 200 < CONCURRENCY) {
            ...
            log(`  ${done}/${target.length} item | ${priceRows} baris harga | ${rate.toFixed(1)} item/detik | sisa ~${left} menit`);
        }
    ...
    return {
        ...
        remaining: pending.length - target.length,
        cursor: finalCursor, done: finished, durationMs: Date.now() - started,
    };
}
