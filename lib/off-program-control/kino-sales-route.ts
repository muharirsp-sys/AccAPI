...
const XLSX_MIMES = new Set(["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/octet-stream", ""]);
const FIELDS = ["accurateFile", "principalFile"] as const;

export class UploadError extends Error {
  readonly status: 400 | 413 | 422;
  constructor(message: string, status: 400 | 413 | 422) { super(message); this.status = status; }
}
...
export function validateUploadForm(form: FormData): [File, File] {
  for (const key of form.keys()) if (!(FIELDS as readonly string[]).includes(key)) throw new UploadError(`Field upload tidak dikenal: ${key}.`, 400);
...
interface HandlerDependencies {
  ...
  reconcile(accurate: Uint8Array, principal: Uint8Array, mapping: Uint8Array): unknown;
}

function isZip(value: Uint8Array): boolean { return value.length >= 4 && value[0] === 0x50 && value[1] === 0x4b; }
...
export function createKinoSalesPostHandler(deps: HandlerDependencies) {
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      const [accurate, principal] = await Promise.all([accurateFile.arrayBuffer(), principalFile.arrayBuffer()]).then((values) => values.map((value) => new Uint8Array(value)));
      ...
      if (error && typeof error === "object" && "code" in error && error.code === "ENOENT") return Response.json({ error: "Master mapping KINO tidak tersedia." }, { status: 500 });
