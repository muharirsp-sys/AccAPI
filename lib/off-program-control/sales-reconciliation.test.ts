...
const kinoMapping = workbook({ Mapping_Prd: [["KODE ITEM", "Kode Alias", "Satuan"], ["ITEM-1", "P-1", "BTL"], ["ITEM-2", "P-2", "TUBE"]], Mapping_Customer: [["Code Kino", "Code Internal"], ["KC-1", "C-1"]], Mapping_Sls: [["SLSMAN_ID", "Code Internal"], ["KS-1", "S-1"]] });
const kinoResult = reconcileKinoSales(accurate, kino, kinoMapping, { valueTolerance: 1 });
assert.equal(kinoResult.summary.MATCH, 1);
assert.equal(kinoResult.summary.QTY_AND_VALUE_MISMATCH, 1);
assert.deepEqual(kinoResult.results.find((row) => row.internalProductCode === "ITEM-2")?.amountDifferences, [
  { component: "gross", accurate: 210, kino: 200, difference: 10 }, { component: "discount", accurate: 5, kino: 0, difference: 5 }, { component: "dpp", accurate: 205, kino: 200, difference: 5 }, { component: "tax", accurate: 24, kino: 22, difference: 2 }, { component: "net", accurate: 228, kino: 222, difference: 6 },
]);
...
const godrej = workbook({ Sheet1: [
  ["IV_NO", "IV_DATE", "CS_NO", "PS_NO", "INV_NO", "IV_TOTPCS", "IV_PRICE", "IV_DISC1", "IV_FRA", "AR_AMT", "IV_DISC2"],
  ["FK/BFG-10025410", 46216, "FGC-1", "FGM-1", "40043482", 6, 1200, 5, 72, 999999, 0],
] });
const godrejMapping = workbook({ "Pvt Map 1": [
  ["Kode Pcpl", "Kode BARANG Win2", "SATUAN Fix Win"],
  ["40043482", "ITEM-G", "BTL"],
  ["40043482", "ITEM-LAIN", "BTL"],
  [null, "(blank)", null],
  ["(blank)", "", 0],
] });
...
const unsupportedAdjustment = workbook({ Sheet1: [
  ["IV_NO", "IV_DATE", "CS_NO", "PS_NO", "INV_NO", "IV_TOTPCS", "IV_PRICE", "IV_DISC1", "IV_FRA", "IV_DISC2"],
  ["FK/BFG-10025410", 46216, "FGC-1", "FGM-1", "40043482", 6, 1200, 5, 72, 1],
] });
assert.throws(() => reconcileGodrejSales(godrejAccurate, unsupportedAdjustment, godrejMapping), /IV_DISC2 belum memiliki aturan/);
console.log("OK — KINO tetap stabil; Godrej memakai REM/IV_NO, IV_TOTPCS, mapping, dan rumus nilai.");
